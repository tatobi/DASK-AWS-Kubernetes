...
log_group_name = __REPLACE_CWG__
...
[/tmp/init-kops.log]
file = /tmp/init-kops.log
log_group_name = __REPLACE_CWG__
log_stream_name = {instance_id}.init-kops
datetime_format = %b %d %H:%M:%S
...
[/var/log/bastion]
log_stream_name = {instance_id}.hostlog
datetime_format = %b %d %H:%M:%S
EOF
    export Region=`curl http://169.254.169.254/latest/meta-data/placement/availability-zone | rev | cut -c 2- | rev`
    cat /tmp/groupname.txt >> ~/cloudwatchlog.conf

    sed -i 's/__REPLACE_CWG__/'$CWG'/g' ~/cloudwatchlog.conf
...
echo $BASTION_LOGFILE
if [[ ! -e $BASTION_LOGFILE ]];
then
